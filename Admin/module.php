...
<title>ISUtoLearn - Admin Modules</title>
...
<style>
	...
	.sidebar-modal { 
		...
		z-index: 50; 
		/* CHANGED: Initial transform is now handled by Tailwind's translate-x-full */
		transition: transform 0.3s ease; /* Ensure transition is smooth */
		overflow-y: auto; 
		box-shadow: -4px 0 12px rgba(0,0,0,0.2); 
		padding: 1.5rem;
	}
	/* CHANGED: This class is now used to trigger the slide-in */
	.sidebar-modal.show { 
...
<div class="flex-1 flex flex-col overflow-y-auto ">
	...
	<div id="addModuleModal" class="fixed inset-0 z-50 hidden">
		...
		<div class="sidebar-modal translate-x-full" id="modalContent"> 
	...
	<div id="editModuleModal" class="fixed inset-0 z-50 hidden">
		...
		<div class="sidebar-modal translate-x-full" id="editModalContent">
	...
	<script>
		...
		function openModal() { 
			// 1. Make the modal container visible
			modal.classList.remove('hidden'); 
			
			// 2. Wait a moment, then remove the 'translate-x-full' utility 
			// and add the 'show' class to trigger the CSS transition to translateX(0)
			setTimeout(() => {
				modalContent.classList.remove('translate-x-full'); // Remove Tailwind utility
				modalContent.classList.add('show'); // Add custom CSS class to trigger slide
			}, 10);
		}

		function closeModal() { 
			// 1. Remove the 'show' class to trigger the CSS transition to slide out
			modalContent.classList.remove('show'); 
			
			// 2. After the transition time (300ms), hide the modal container and reset the initial position
			setTimeout(() => {
				modal.classList.add('hidden');
				modalContent.classList.add('translate-x-full'); // Reset initial position for next time
		...
		// --- Edit Modal Logic (Fixed to use the same transition logic) ---
		...
		editBtns.forEach(btn => {
			btn.addEventListener('click', e => {
				...
				// Open edit modal (similar logic to Add Modal)
				editModal.classList.remove('hidden');
				setTimeout(() => {
					editModalContent.classList.remove('translate-x-full');
